...
export const concepts: Concept[] = [
  ...
  },
  {
    ...
    elevator:
      "Lyapunov drift analysis is how you prove a system is stable without simulating every possible scenario. You define a 'potential energy' function of the system state (in our case, the total queue backlog squared). Then you show that at each time step, the expected change in this energy (the 'drift') is negative — meaning the system trends toward emptying its queues. If drift is negative, queues stay bounded, which means every request eventually gets served. Pura uses this to prove that backpressure routing achieves the maximum possible throughput.",
    detail:
