...
order: 6
description: Stake splitting analysis, MEV protection, slashing
---

## Stake Splitting and Sybil Friction

Under permissionless unlinkable identities, BPE does not claim full Sybil resistance. The per-address cap $\text{cap}(S) = \sqrt{S/u}$ is concave, which dampens concentration by a single large staker, but splitting total stake $S$ across $n$ addresses increases aggregate capacity by $\sqrt{n}$. Economic friction comes from two sources: a minimum per-sink stake $S_{\min}$ and independent slashing exposure per identity.
