...
Sinks deposit ERC-20 tokens to stake. The capacity cap is computed as $\text{cap}(k) = \sqrt{\text{stake}(k) / u}$ where $u$ is the stake unit parameter (set to $10^{18}$). This concave function dampens per-address capacity concentration: doubling stake increases capacity by only $\sqrt{2}$, not 2. Under permissionless unlinkable identities, splitting total stake $S$ equally across $n$ addresses yields aggregate capacity $\sqrt{n} \cdot \sqrt{S/u}$, which is larger than the single-address cap $\sqrt{S/u}$ by a factor of $\sqrt{n}$. Sybil friction is enforced through two additional mechanisms: a minimum per-sink stake $S_{\min}$ limits the number of viable splits to $\lfloor S/S_{\min} \rfloor$, and each identity bears independent slashing exposure, making the economics of splitting deteriorate as split count grows (see Security Analysis).
