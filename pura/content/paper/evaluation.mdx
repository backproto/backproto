...
## E3: Stake Splitting Analysis

![Stake splitting analysis. Raw aggregate capacity grows as √n (the concave cap is anti-whale but not anti-split), while net profit decreases because per-identity minimum stake costs accumulate linearly.](/paper/figures/sybil.png)

*Stake splitting analysis. Raw aggregate capacity grows as $\sqrt{n}$ while net profit decreases as per-identity minimum stake costs accumulate linearly, becoming unprofitable at moderate $n$.*

We fix total attacker stake $S$ and split into $n \in \{1, 2, 5, 10, 20\}$ identities. Each identity pays $S_{\min}$ overhead. Total capacity scales as $\sqrt{n}$ while cost scales linearly. The attack becomes unprofitable at moderate $n$ — not because splitting reduces capacity (it increases it by $\sqrt{n}$), but because per-identity costs grow linearly while marginal capacity gains shrink. This validates the frictional analysis in Security Analysis.
