...
const taskTypeIds = TASK_TYPES.map((t) =>
  keccak256(encodePacked(["string"], [t.name])),
);
...
interface CycleRow {
  cycle: number;
  taskType: string;
  gasAttestation: bigint;
  gasQueueLoad: bigint;
  gasPricingEpoch: bigint;
  gasCompletion: bigint;
  gasCompletionEpoch: bigint;
  gasRebalance: bigint;
  price: bigint;
  baseFee: bigint;
...
async function setup() {
  ...
  if (currentStake < MIN_STAKE) {
    ...
    await safeTx(
      () => stake.stake(walletClient, addrs, diff),
      `stake ${formatEther(diff)} BPE`,
    );
  } else {
  ...
  for (let i = 0; i < TASK_TYPES.length; i++) {
    ...
    await safeTx(
      ...
      `registerTaskType(${t.name})`,
    );
    await safeTx(
      () => pool.createPool(walletClient, addrs, id),
      `createPool(${t.name})`,
    );
    await safeTx(
      () => sink.registerSink(walletClient, addrs, id, t.capacity),
      `registerSink(${t.name}, cap=${t.capacity})`,
    );
  }
}
...
async function runCycle(cycle: number) {
  ...
  for (let i = 0; i < TASK_TYPES.length; i++) {
    ...
    const rAttest = await safeTx(
      () => aggregator.submitBatch(walletClient, addrs, [signed]),
      `attestation(${t.name}, cap=${noisedCapacity})`,
    );

    // --- Queue load ---
    const queueLoad = t.queueBase + BigInt(Math.floor(Math.random() * 10));
    const rQueue = await safeTx(
      () => pricing.reportQueueLoad(walletClient, addrs, id, account.address, queueLoad),
      `queueLoad(${t.name}, load=${queueLoad})`,
    );

    // --- Pricing epoch ---
    const rPricing = await safeTx(
      () => pricing.advancePricingEpoch(walletClient, addrs, id),
      `pricingEpoch(${t.name})`,
    );

    // --- Completion (80% probability) ---
    let rCompletion: TransactionReceipt | null = null;
    const doComplete = Math.random() < 0.8;
    if (doComplete) {
      const taskId = keccak256(
        encodePacked(["string", "uint256", "uint256"], [t.name, now, BigInt(cycle)]),
      );
      const receipt = {
        taskTypeId: id,
        sink: account.address,
      ...
      const sinkSig = await signCompletionReceipt(
        walletClient, CHAIN_ID, addrs.completionTracker, receipt,
      );
      rCompletion = await safeTx(
        () => completion.recordCompletion(walletClient, addrs, id, account.address, taskId, sinkSig),
        `completion(${t.name})`,
      );
    }

    // --- Completion epoch ---
    const rCompEpoch = await safeTx(
      () => completion.advanceCompletionEpoch(walletClient, addrs, id, account.address),
      `completionEpoch(${t.name})`,
    );

    // --- Rebalance ---
    let rRebalance: TransactionReceipt | null = null;
    const needsReb = await pool.needsRebalance(publicClient, addrs, id);
    if (needsReb) {
      rRebalance = await safeTx(
        () => pool.rebalance(walletClient, addrs, id),
        `rebalance(${t.name})`,
      );
    }
    ...
    console.log(
      `  => price=${price} baseFee=${baseFee} compRate=${completionRate} units=${poolUnits}`,
    );
  }
}
...
function writeCSV() {
  const header =
    "cycle,taskType,gasAttestation,gasQueueLoad,gasPricingEpoch,gasCompletion,gasCompletionEpoch,gasRebalance,price,baseFee,completionRate,poolUnits";
  const lines = rows.map(
    (r) =>
      `${r.cycle},${r.taskType},${r.gasAttestation},${r.gasQueueLoad},${r.gasPricingEpoch},${r.gasCompletion},${r.gasCompletionEpoch},${r.gasRebalance},${r.price},${r.baseFee},${r.completionRate},${r.poolUnits}`,
  );
...
function printSummary() {
  ...
  for (let i = 0; i < TASK_TYPES.length; i++) {
    ...
    if (lastRow) {
      console.log(
        `    ${name.padEnd(20)} price=${lastRow.price}  baseFee=${lastRow.baseFee}  compRate=${lastRow.completionRate}  units=${lastRow.poolUnits}`,
      );
    }
  }
