...
async function main() {
  ...
  console.log("\nStep 5: Report queue load & read price");
  const txQueue = await pricing.reportQueueLoad(
    walletClient, addrs, TASK_TYPE_ID, account.address, 10n,
  );
  ...
  try {
    const txComplete = await completion.recordCompletion(
      walletClient, addrs, TASK_TYPE_ID, account.address, taskId, sinkSig,
    );
